...
namespace J_Project.Equipment
{
    /**
     *  @brief 전류 측정용 DMM 장비
     *  @details 전류 측정용 DMM에 대한 일련의 관리를 담당하는 클래스
     *
     *  @author SSW
     *  @date 2020.02.25
     *  @version 1.0.0
     */
    public class Dmm2 : Equipment
    {
        ...
        private Dmm2()
        {
            ...
            {
                double result = Math.Round(RealDcVolt() * 1000, 3);
                e.Result = result;
            });
        ...
        public static Dmm2 GetObj()
        {
            if (_Dmm2 == null) _Dmm2 = new Dmm2();
            return _Dmm2;
        }
        #endregion
        ...
        {
            // 송신
            TryResultFlag commResult = CommModule.CommSend("MEASure:VOLTage:DC? 0.1");
