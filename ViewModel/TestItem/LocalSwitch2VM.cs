...
{
    /**
     *  @brief Local Switch 체크 테스트 클래스(출하용)
     *  @details Local Switch 체크와 관련된 시퀀스 및 UI관련을 담당하는 클래스
     *
     *  @author SSW
     *  @date 2020.02.25
     *  @version 1.0.0
     */
    internal class LocalSwitch2VM : AllTestVM
    {
        private enum Seq
        {
            AC_ON,
        ...
        public override StateFlag TestSeq(int caseNumber, int stepNumber, ref int jumpStepNum)
        {
            ...
            switch (stepName)
            {
                ...
                case Seq.LOCAL_CHANGE:  // 로컬 변경
                    ...
                    {
                        MessageBoxResult msgResult = MessageBox.Show("스위치를 로컬로 바꿔주세요", "스위치 변경", MessageBoxButton.OKCancel);
                ...
                case Seq.RESULT_SAVE: // 결과 저장
                    ...
                    result = ResultDataSave((int)SecondTestOrder.LocalSwitch, TestName, resultData);
