...
{
    /**
     *  @brief AC 고전압 테스트 클래스
     *  @details AC 고전압과 관련된 시퀀스 및 UI관련을 담당하는 클래스
     *
     *  @author SSW
     *  @date 2020.02.25
     *  @version 1.0.0
     */
    internal class AcHighVM : AllTestVM
    {
        private enum Seq
        {
            AC_INIT,
        ...
        public override StateFlag TestSeq(int caseNumber, int stepNumber, ref int jumpStepNum)
        {
            ...
            switch (stepName)
            {
                case Seq.AC_INIT: // 초기 AC 설정 및 ON
                ...
                case Seq.AC_HIGH_CHECK: // AC 상승 설정
                    TestLog.AppendLine("[ AC 상승 ]");
        ...
        private StateFlag AcHighCheck(int caseNum, double acUp, double errRate, ref (string, string) resultData)
        {
            ...
            double maxAc = acUp + errRate;

            TestLog.AppendLine($"- 검사 범위 : {minAc} ~ {maxAc}");

            for (double inputAc = minAc; inputAc <= maxAc; inputAc++)
            {
                // AC 단계별 상승
                StateFlag result = AcSourceSet(inputAc);
                TestLog.AppendLine($"- AC 상승 : {pm.AcVolt} / {result}");
                if (result != StateFlag.PASS)
                {
                ...
                // Fault 발생 확인
                bool faultFlag = Rectifier.GetObj().Flag_OverAcInVolt;
                if (faultFlag == true)
                {
