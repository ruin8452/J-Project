...
{
    /**
     *  @brief 출력 저전압 보호 테스트 클래스
     *  @details 출력 저전압 보호와 관련된 시퀀스 및 UI관련을 담당하는 클래스
     *
     *  @author SSW
     *  @date 2020.02.25
     *  @version 1.0.0
    ...
    internal class OutputLowVM : AllTestVM
    {
        ...
        public OutputLowVM(int caseNum)
        {
            TestLog = new StringBuilder();
            CaseNum = caseNum;

            TestOrterNum = (int)FirstTestOrder.OutputLow + caseNum;
            TotalStepNum = (int)Seq.END_TEST + 1;

            OutputLow = new 출력_저전압_보호();
            OutputLow = (출력_저전압_보호)Test.Load(OutputLow, CaseNum);
        ...
        public override StateFlag TestSeq(int stepNumber, ref int jumpStepNum)
        {
            ...
            switch (stepName)
            {
                case Seq.AC_ON: // 초기 AC 설정
                    ...
                    if (Math.Abs(OutputLow.AcVolt - acVolt) <= AC_ERR_RANGE)
                    {
                    ...
                    {
                        result = AcSourceSet(OutputLow.AcVolt, OutputLow.AcCurr, OutputLow.AcFreq);
                    ...
                    else
                    {
                        ...
                        result = AcCtrlWin(OutputLow.AcVolt, AC_ERR_RANGE, AcCheckMode.NORMAL);
                        TestLog.AppendLine($"- AC 전원 결과 : {result}\n");
                ...
                case Seq.DELAY1:
                    ...
                    Util.Delay(OutputLow.Delay1);
                    result = StateFlag.PASS;
                    break;

                case Seq.OUT_LOW_CHECK:
                    TestLog.AppendLine("[ 기능 검사 ]");
                    result = RectOutLowCheck(OutputLow.DcOutVolt, OutputLow.DcErrRate, ref resultData);
                ...
                case Seq.NEXT_TEST_DELAY:
                    ...
                    Util.Delay(OutputLow.NextTestWait);
                    result = StateFlag.PASS;
                    break;
         ...
         *  @return StateFlag - 수행 결과
         */
        private StateFlag RectOutLowCheck(double outDrop, double errRate, ref (string, string) resultData)
        {
